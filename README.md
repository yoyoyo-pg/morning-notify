...
毎朝6時と毎晩21時にDiscordへ通知するPythonアプリ。このボットの名前は**ショーンK**。朝はウェザー・スケジュール・ニュース・ジャーナルを、夜はデイリー・リフレクションのリマインダーを送る。
...
| カード | 色 | 内容 |
|--------|-----|------|
| 📓 デイリー・リフレクション | 紫 | アクション・アイテムとアチーブメントをレコーディングするよう促すリマインダー |
...
名古屋・愛知エリアの直近イベントを月・木の朝6時にDiscordへ通知するボット。ショーンKと同じ `src/` 配下（`src/eventkun/`）で管理する。
...
```bash
DISCORD_WEBHOOK_URL=https://... PYTHONPATH=src/reminkun:src/eventkun python src/eventkun/main.py
```
